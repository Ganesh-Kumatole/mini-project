...
const SectionTitle = ({ icon, children }: { icon: string; children: React.ReactNode }) => (
  <div className="flex items-center gap-2 mb-6">
    ...
    <h2 className="text-lg font-semibold text-text-primary-light dark:text-text-primary-dark">{children}</h2>
  </div>
);
...
const StatCard = ({
  icon, iconColor, label, value, sub, subColor, id,
}: {
  icon: string; iconColor: string; label: string; value: string;
  sub?: string; subColor?: string; id?: string;
}) => (
  <div id={id} className="bg-surface-light dark:bg-surface-dark p-6 rounded-xl shadow-sm border border-border-light dark:border-border-dark flex flex-col gap-3 transition-all hover:shadow-md">
    <div className="flex items-center justify-between">
      ...
      <span className="text-xs font-medium text-text-secondary-light dark:text-text-secondary-dark uppercase tracking-wider">{label}</span>
    </div>
    <p className="text-2xl font-bold text-text-primary-light dark:text-text-primary-dark leading-tight">{value}</p>
    {sub && <p className={`text-xs font-medium ${subColor ?? 'text-text-secondary-light dark:text-text-secondary-dark'}`}>{sub}</p>}
  </div>
);
...
const AIChatSection = ({ stats }: { stats: FinancialStats }) => {
  ...
  const sendMessage = useCallback(async (question: string) => {
    ...
    try {
      ...
      setMessages((prev) => prev.map((m, i) => i === prev.length - 1 ? { role: 'assistant', content: answer } : m));
    } catch {
      setMessages((prev) => prev.map((m, i) => i === prev.length - 1 ? { role: 'assistant', content: 'Sorry, couldn\'t get a response. Try again.' } : m));
    } finally {
      setIsAsking(false);
      setTimeout(scrollToBottom, 50);
    }
  ...
  return (
    <SectionCard className="flex flex-col overflow-hidden !p-0">
      <div className="flex items-center gap-2 px-6 pt-6 pb-4 border-b border-border-light dark:border-border-dark">
        ...
        <h2 className="text-lg font-semibold text-text-primary-light dark:text-text-primary-dark">Ask Your Finance AI</h2>
        {messages.length > 0 && (
          <button onClick={() => setMessages([])} className="ml-auto text-xs text-text-secondary-light dark:text-text-secondary-dark hover:text-red-500 transition-colors">Clear</button>
        )}
      </div>
      <div className="overflow-y-auto px-6 py-4 space-y-4 min-h-[220px] max-h-[340px]">
        {messages.length === 0 ? (
          <div className="flex flex-col gap-3">
            <p className="text-sm text-text-secondary-light dark:text-text-secondary-dark">Try one of these:</p>
            <div className="flex flex-wrap gap-2">
              {SUGGESTED_QUESTIONS.map((q) => (
                <button key={q} onClick={() => sendMessage(q)} className="text-xs px-3 py-1.5 rounded-full border border-indigo-200 dark:border-indigo-700 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 hover:bg-indigo-100 dark:hover:bg-indigo-900/60 transition-colors">{q}</button>
              ))}
            </div>
          </div>
        ) : messages.map((msg, i) => (
          <div key={i} className={`flex gap-2 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            {msg.role === 'assistant' && <span className="material-icons text-indigo-500 text-base mt-1 flex-shrink-0">auto_awesome</span>}
            <div className={`max-w-[80%] px-4 py-2.5 rounded-2xl text-sm leading-relaxed ${msg.role === 'user' ? 'bg-indigo-600 text-white rounded-br-sm' : 'bg-gray-100 dark:bg-gray-800 text-text-primary-light dark:text-text-primary-dark rounded-bl-sm'}`}>
              {msg.loading ? (
      ...
      <div className="px-6 pb-6 pt-2">
        <div className="flex items-center gap-2 bg-gray-100 dark:bg-gray-800 rounded-xl px-4 py-2.5 border border-transparent focus-within:border-indigo-400 dark:focus-within:border-indigo-600 transition-colors">
          <input
            id="ai-chat-input" type="text" value={input}
            onChange={(e) => setInput(e.target.value)}
            onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendMessage(input); } }}
            placeholder="Type a question…" disabled={isAsking}
          ...
          />
          <button id="ai-chat-send-btn" onClick={() => sendMessage(input)} disabled={!input.trim() || isAsking} className="p-1.5 rounded-lg bg-indigo-600 text-white hover:bg-indigo-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors flex-shrink-0">
            <span className="material-icons text-sm">{isAsking ? 'hourglass_empty' : 'send'}</span>
          </button>
        </div>
      </div>
    </SectionCard>
...
const SavingsGoalSection = ({ currentSaved }: { currentSaved: number }) => {
  ...
  const barColor = pct >= 100 ? 'bg-emerald-500' : pct >= 60 ? 'bg-indigo-500' : pct >= 30 ? 'bg-amber-400' : 'bg-red-400';
    ...
    <SectionCard>
      ...
      {goal === 0 ? (
        <div className="flex flex-col items-center gap-4 py-4">
          <span className="material-icons text-4xl text-gray-300 dark:text-gray-600">savings</span>
        ...
        <div className="space-y-4">
          ...
          <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-3 overflow-hidden">
            ...
            >
              {pct > 15 && (
                <span className="absolute right-2 top-1/2 -translate-y-1/2 text-white text-xs font-bold">{pct.toFixed(0)}%</span>
              )}
            </div>
          </div>
          <button
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex gap-2">
            <input
              ...
              placeholder="e.g. 500"
              className="flex-1 px-4 py-2 rounded-lg border border-border-light dark:border-border-dark bg-gray-50 dark:bg-gray-800 text-text-primary-light dark:text-text-primary-dark text-sm outline-none focus:border-indigo-400 dark:focus:border-indigo-600 transition-colors"
            />
            <button onClick={saveGoal} className="px-4 py-2 rounded-lg bg-indigo-600 text-white text-sm font-medium hover:bg-indigo-700 transition-colors">Save</button>
            <button onClick={() => setEditing(false)} className="px-3 py-2 rounded-lg border border-border-light dark:border-border-dark text-sm text-text-secondary-light dark:text-text-secondary-dark hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">Cancel</button>
          </div>
        </div>
      )}
    </SectionCard>
...
export const Insights = () => {
  ...
  const {
    currentIncome, currentExpense, savingsRate, monthOverMonthChange,
    topCategories, budgetHealth, budgetAlerts, biggestExpense,
    monthlyTrend, projectedMonthlySpend, daysElapsed, daysInMonth,
    anomalies, weekdaySpend, weekendSpend, weekdayPct, weekendPct,
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const canvas = await html2canvas(exportRef.current, { scale: 2, useCORS: true, backgroundColor: isDark ? '#111827' : '#ffffff', logging: false });
      ...
      let yPos = 10, remaining = ih;
      while (remaining > 0) {
        pdf.addImage(imgData, 'PNG', 10, yPos, iw, ih);
        remaining -= ph - 20;
        if (remaining > 0) { pdf.addPage(); yPos = -(ih - remaining) - 10; }
      }
  ...
  const barColors = isDark
    ? ['rgba(129,140,248,0.85)', 'rgba(251,146,60,0.85)', 'rgba(251,113,133,0.85)', 'rgba(52,211,153,0.85)', 'rgba(196,181,253,0.85)']
    : ['rgba(99,102,241,0.8)', 'rgba(249,115,22,0.8)', 'rgba(244,63,94,0.8)', 'rgba(52,211,153,0.8)', 'rgba(167,139,250,0.8)'];

  // ── Derived display values ──
  const momDisplay = monthOverMonthChange !== null
    ? `${monthOverMonthChange >= 0 ? '+' : ''}${monthOverMonthChange.toFixed(1)}% vs last month`
    : 'No previous month data';
  const momColor = monthOverMonthChange === null ? '' : monthOverMonthChange > 0 ? 'text-red-500' : 'text-emerald-500';
  const momIcon = monthOverMonthChange !== null && monthOverMonthChange > 0 ? 'trending_up' : 'trending_down';
  const savingsColor = savingsRate >= 20 ? 'text-emerald-500' : savingsRate >= 10 ? 'text-amber-500' : 'text-red-500';

  // ── Category breakdown bar chart ──
  const catBarData = {
    labels: topCategories.map((c) => c.name),
    datasets: [{ label: 'Spent', data: topCategories.map((c) => c.amount), backgroundColor: barColors, borderRadius: 6, borderSkipped: false as const }],
  };
  ...
  const bvaData = {
    labels: bvaLabels,
    datasets: [
      { label: 'Budget Limit', data: budgetHealth.map((b) => b.limitAmount), backgroundColor: isDark ? 'rgba(99,102,241,0.35)' : 'rgba(99,102,241,0.25)', borderColor: isDark ? 'rgba(129,140,248,0.8)' : 'rgba(99,102,241,0.8)', borderWidth: 1.5, borderRadius: 4, borderSkipped: false as const },
      { label: 'Actual Spent', data: budgetHealth.map((b) => b.spentAmount), backgroundColor: budgetHealth.map((b) => b.percentUsed >= 90 ? (isDark ? 'rgba(251,113,133,0.85)' : 'rgba(244,63,94,0.8)') : b.percentUsed >= 60 ? (isDark ? 'rgba(251,191,36,0.8)' : 'rgba(245,158,11,0.75)') : (isDark ? 'rgba(52,211,153,0.8)' : 'rgba(16,185,129,0.75)')), borderRadius: 4, borderSkipped: false as const },
    ],
  };
  const bvaOptions = {
    responsive: true, maintainAspectRatio: false,
  ...
  const momLineData = {
    labels: momLabels,
    datasets: [
      { label: 'Income', data: monthlyTrend.map((m) => m.income), borderColor: isDark ? '#34d399' : '#059669', backgroundColor: isDark ? 'rgba(52,211,153,0.12)' : 'rgba(5,150,105,0.08)', tension: 0.4, fill: true, pointRadius: 3, pointHoverRadius: 5 },
      { label: 'Expenses', data: monthlyTrend.map((m) => m.expense), borderColor: isDark ? '#f87171' : '#dc2626', backgroundColor: isDark ? 'rgba(248,113,113,0.1)' : 'rgba(220,38,38,0.07)', tension: 0.4, fill: true, pointRadius: 3, pointHoverRadius: 5 },
    ],
  };
  ...
  const financialStats: FinancialStats = {
    totalIncome: currentIncome, totalExpense: currentExpense, savingsRate,
    monthOverMonthChange: monthOverMonthChange ?? 0,
    topCategories, budgetAlerts: budgetAlerts.map((b) => ({ category: b.category, percentUsed: b.percentUsed })),
    biggestExpense,
  };
  ...
  if (loading) {
    return (
      <div className="max-w-7xl mx-auto space-y-6 animate-pulse">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">{[1,2,3,4].map((i) => <Shimmer key={i} className="h-36" />)}</div>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">{[1,2,3,4].map((i) => <Shimmer key={i} className="h-64" />)}</div>
      </div>
    );
  }
  ...
  if (!loading && currentIncome === 0 && currentExpense === 0) {
    return (
      <div className="max-w-7xl mx-auto">
        <h1 id="insights-heading" className="text-2xl font-bold text-text-primary-light dark:text-text-primary-dark mb-8">AI-Powered Insights</h1>
        <SectionCard className="flex flex-col items-center gap-4 p-12 text-center">
          ...
          <h2 className="text-lg font-semibold text-text-primary-light dark:text-text-primary-dark">No transactions yet this month</h2>
          <p className="text-sm text-text-secondary-light dark:text-text-secondary-dark max-w-sm">Add some transactions and budgets to unlock personalised AI insights.</p>
        </SectionCard>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <h1 id="insights-heading" className="text-2xl font-bold text-text-primary-light dark:text-text-primary-dark">AI-Powered Insights</h1>
        <div className="flex items-center gap-2">
          <button id="export-pdf-btn" onClick={handleExport} disabled={exporting} className="inline-flex items-center gap-2 px-4 py-2 rounded-lg border border-border-light dark:border-border-dark text-sm font-medium text-text-primary-light dark:text-text-primary-dark hover:bg-gray-50 dark:hover:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
            <span className={`material-icons text-base ${exporting ? 'animate-pulse' : ''}`}>picture_as_pdf</span>
            {exporting ? 'Exporting…' : 'Export PDF'}
          </button>
          <button id="refresh-ai-btn" onClick={refreshAI} disabled={aiLoading} className="inline-flex items-center gap-2 px-4 py-2 rounded-lg border border-border-light dark:border-border-dark text-sm font-medium text-text-primary-light dark:text-text-primary-dark hover:bg-gray-50 dark:hover:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
      ...
      <div ref={exportRef} className="space-y-8">
        ...
        {aiLoading ? (
          <div className="bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-950/40 dark:to-purple-950/40 border border-indigo-200 dark:border-indigo-800 rounded-xl p-6 space-y-3">
            <div className="flex items-center gap-3">
              ...
              <span className="font-semibold text-indigo-700 dark:text-indigo-300">AI is analysing your finances…</span>
            </div>
            <Shimmer className="h-4 w-full" /><Shimmer className="h-4 w-5/6" /><Shimmer className="h-4 w-4/6" />
          </div>
        ) : aiError ? (
          <div className="bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 rounded-xl p-6 flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-amber-700 dark:text-amber-300 mt-1">{aiError} — showing computed insights only.</p>
            </div>
          </div>
        ) : aiSummary ? (
          <div className="bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-950/40 dark:to-purple-950/40 border border-indigo-200 dark:border-indigo-800 rounded-xl p-6">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="font-semibold text-indigo-700 dark:text-indigo-300 text-sm uppercase tracking-wider">AI Financial Summary</span>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <SectionCard>
            ...
            <div className="flex flex-col gap-4">
              ...
              {budgetHealth.length > 0 && (() => {
                const totalLimit = budgetHealth.reduce((s, b) => s + b.limitAmount, 0);
          ...
          <SectionCard>
            ...
            {weekdaySpend + weekendSpend === 0 ? (
              <p className="text-sm text-text-secondary-light dark:text-text-secondary-dark">No expense data yet.</p>
            ) : (
              <div className="flex flex-col gap-4">
                {/* Stacked visual bar */}
                <div className="w-full h-5 rounded-full overflow-hidden flex">
                  <div className="bg-indigo-500 h-full transition-all duration-700" style={{ width: `${weekdayPct}%` }} />
                  <div className="bg-orange-400 h-full transition-all duration-700" style={{ width: `${weekendPct}%` }} />
                </div>
                ...
                {weekendPct > 50 && (
                  <p className="text-xs text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/30 rounded-lg px-3 py-2">
                    💡 Over half your spending happens on weekends. Consider planning weekend activities with a budget.
                  </p>
                )}
              </div>
            )}
        ...
        {budgetHealth.length > 0 && (
          <SectionCard>
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-2">
                <span className="material-icons text-indigo-500 text-xl">account_balance</span>
                <h2 className="text-lg font-semibold text-text-primary-light dark:text-text-primary-dark">Budget Health</h2>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-5">
              {budgetHealth.map((b) => <BudgetRow key={b.id} {...b} />)}
            </div>
          </SectionCard>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <SectionCard>
            <SectionTitle icon="show_chart">6-Month Income vs. Expense</SectionTitle>
            <div className="h-56">
              <Line key={`mom-${theme}`} data={momLineData} options={momLineOptions} />
            </div>
          </SectionCard>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <SectionCard>
            <SectionTitle icon="pie_chart">Category Breakdown (This Month)</SectionTitle>
            {topCategories.length === 0 ? (
              <div className="h-56 flex items-center justify-center">
                <p className="text-sm text-text-secondary-light dark:text-text-secondary-dark">No expense data for this month</p>
              </div>
            ) : (
              <div className="h-56"><Bar key={`cat-${theme}`} data={catBarData} options={catBarOptions} /></div>
            )}
          </SectionCard>

          <SectionCard>
            <SectionTitle icon="lightbulb">AI Tips &amp; Recommendations</SectionTitle>
            {aiLoading ? (
              <div className="space-y-4">
                {[1,2,3].map((i) => (
                  <div key={i} className="flex items-start gap-3">
                    <Shimmer className="h-5 w-5 rounded-full flex-shrink-0 mt-0.5" />
                    <div className="flex-1 space-y-1.5"><Shimmer className="h-4 w-full" /><Shimmer className="h-4 w-4/5" /></div>
                  </div>
                ))}
              </div>
            ) : aiTips.length > 0 ? (
              <ul className="space-y-4">
                {aiTips.map((tip, i) => (
                  <li key={i} className="flex items-start gap-3 group">
                    <span className="inline-flex items-center justify-center h-6 w-6 rounded-full bg-indigo-100 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400 text-xs font-bold flex-shrink-0 mt-0.5 group-hover:scale-110 transition-transform">{i + 1}</span>
                    <p className="text-sm text-text-primary-light dark:text-text-primary-dark leading-relaxed">{tip}</p>
                  </li>
                ))}
              </ul>
            ) : (
              <div className="flex flex-col items-center justify-center gap-3 py-8 text-center">
                ...
                <p className="text-sm text-text-secondary-light dark:text-text-secondary-dark">{aiError ? 'AI tips unavailable right now. Try refreshing.' : 'AI tips will appear here once your data is analysed.'}</p>
              </div>
            )}
          </SectionCard>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          {/* Anomaly Detection */}
          <SectionCard>
            <SectionTitle icon="notifications_active">Anomaly Detection</SectionTitle>
            {anomalies.length === 0 ? (
              <div className="flex flex-col items-center gap-3 py-6 text-center">
                <span className="material-icons text-3xl text-emerald-400">check_circle</span>
                <p className="text-sm text-text-secondary-light dark:text-text-secondary-dark">No unusual transactions detected this month. Your spending looks normal.</p>
              </div>
            ) : (
              <div className="space-y-3">
                <p className="text-xs text-text-secondary-light dark:text-text-secondary-dark mb-4">
                  {anomalies.length} transaction{anomalies.length > 1 ? 's' : ''} significantly higher than your usual {anomalies.length > 1 ? 'amounts' : 'amount'} in their category.
                </p>
                {anomalies.slice(0, 4).map((a, i) => (
                  <div key={i} className="flex items-start justify-between gap-3 p-3 rounded-lg bg-amber-50 dark:bg-amber-950/20 border border-amber-100 dark:border-amber-900/40">
                    <div className="flex items-start gap-3 min-w-0">
                      <span className="material-icons text-amber-500 text-base flex-shrink-0 mt-0.5">warning_amber</span>
          ...
          <SectionCard>
            ...
            {recurringExpenses.length === 0 ? (
              <div className="flex flex-col items-center gap-3 py-6 text-center">
                <span className="material-icons text-3xl text-gray-300 dark:text-gray-600">event_repeat</span>
                <p className="text-sm text-text-secondary-light dark:text-text-secondary-dark">No recurring expenses detected yet. They appear once you have 2+ months of similar transactions.</p>
              </div>
            ) : (
              <div className="space-y-3">
                ...
                {recurringExpenses.slice(0, 5).map((r, i) => (
                  <div key={i} className="flex items-center justify-between gap-3 py-2.5 border-b border-border-light dark:border-border-dark last:border-0">
                    <div className="flex items-center gap-3 min-w-0">
                      <span className="material-icons text-indigo-400 text-base flex-shrink-0">repeat</span>
                      <div className="min-w-0">
                        <p className="text-sm font-medium text-text-primary-light dark:text-text-primary-dark truncate">{r.description}</p>
