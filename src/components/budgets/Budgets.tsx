...
const EditBudgetModal = ({
  ...
  const [notificationThreshold, setNotificationThreshold] = useState<NotificationThreshold>(75);
  ...
  useEffect(() => {
    if (budget) {
      ...
      setNotificationThreshold(budget.notificationThreshold as NotificationThreshold);
  ...
  const handleSave = async () => {
    ...
    setSaveError(null);
    try {
      await onSave({ id: budget.id, limitAmount: amount, notificationThreshold, period });
      onClose();
    } catch (e: any) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-surface-light dark:bg-surface-dark rounded-2xl shadow-2xl max-w-sm w-full p-6 border border-border-light dark:border-border-dark">
        <div className="flex items-center justify-between mb-5">
          <div>
            <h3 className="text-lg font-bold text-text-primary-light dark:text-text-primary-dark">Edit Budget</h3>
            <p className="text-xs text-text-secondary-light dark:text-text-secondary-dark mt-0.5">{budget.category}</p>
          </div>
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-text-primary-light dark:text-text-primary-dark mb-1.5">Budget Limit</label>
            <div className="relative">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-text-secondary-light dark:text-text-secondary-dark font-medium">{symbol}</span>
              <input
                type="number"
                value={limitAmount}
          ...
          <div>
            <label className="block text-sm font-medium text-text-primary-light dark:text-text-primary-dark mb-1.5">Alert Threshold</label>
            <div className="flex gap-3">
              {[50, 75, 90].map(t => (
                <button
                  key={t}
                  onClick={() => setNotificationThreshold(t as NotificationThreshold)}
          ...
          <div>
            <label className="block text-sm font-medium text-text-primary-light dark:text-text-primary-dark mb-1.5">Period</label>
            <select
              value={period}
...
const DeleteBudgetModal = ({ isOpen, category, onConfirm, onCancel, loading }: {
  isOpen: boolean; category: string; onConfirm: () => void; onCancel: () => void; loading: boolean;
}) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-surface-light dark:bg-surface-dark rounded-2xl shadow-2xl max-w-sm w-full p-6 border border-red-200 dark:border-red-800">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className="text-base font-bold text-text-primary-light dark:text-text-primary-dark">Delete Budget?</h3>
        ...
        <p className="text-sm text-text-secondary-light dark:text-text-secondary-dark mb-5">
          This will permanently delete the <strong>{category}</strong> budget. Your transactions will not be affected.
        </p>
        <div className="flex gap-3">
          <button onClick={onCancel} disabled={loading} className="flex-1 px-4 py-2.5 rounded-lg border border-border-light dark:border-border-dark text-sm font-medium text-text-secondary-light dark:text-text-secondary-dark hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">Cancel</button>
          <button onClick={onConfirm} disabled={loading} className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-red-600 hover:bg-red-700 text-white text-sm font-medium transition-colors disabled:opacity-50">
...
export const Budgets = () => {
  ...
  const spentByBudget = useMemo(() => {
    ...
    for (const budget of budgets) {
      ...
      map[budget.id] = transactions.reduce((s, t) => {
        ...
        if (t.category === budget.category && t.type === 'expense' && d >= start && d <= end) return s + t.amount;
        return s;
      }, 0);
    }
    return map;
  ...
  const totalSpent = useMemo(() => Object.values(spentByBudget).reduce((s, v) => s + v, 0), [spentByBudget]);
  const overBudgetCount = useMemo(() =>
    budgets.filter(b => (spentByBudget[b.id] || 0) > b.limitAmount).length, [budgets, spentByBudget]);
  ...
  useEffect(() => {
    ...
    const check = async () => {
      for (const budget of budgets) {
        ...
        if (shouldNotify(percentage, budget.notificationThreshold, budget.notificationSent, budget.lastNotificationDate)) {
          try {
            await createNotification({
              ...
              actionUrl: '/budgets',
              metadata: { budgetId: budget.id, category: budget.category, percentageUsed: percentage },
            });
            await updateBudget({ id: budget.id, notificationSent: true, lastNotificationDate: new Date(), spentAmount: spent });
            addToast({ type: 'warning', title: 'Budget Alert', message: `${budget.category} budget is ${percentage}% used` });
  ...
  // ── Handlers ──
  const handleEditClick = (budget: Budget) => { setEditingBudget(budget); setShowEditModal(true); };
  const handleDeleteClick = (budget: Budget) => { setDeletingBudget(budget); };
  const handleConfirmDelete = async () => {
    if (!deletingBudget) return;
    setDeleteLoading(true);
    try {
      await deleteBudget(deletingBudget.id);
      addToast({ type: 'success', title: 'Budget deleted', message: `${deletingBudget.category} budget removed.` });
      setDeletingBudget(null);
    } catch (e) { addToast({ type: 'error', title: 'Delete failed', message: 'Could not delete budget.' }); }
    finally { setDeleteLoading(false); }
  };
  const handleSaveEdit = async (data: Partial<Budget> & { id: string }) => {
    await updateBudget(data);
    addToast({ type: 'success', title: 'Budget updated', message: 'Changes saved successfully.' });
  };
  const handleViewDetails = (budget: Budget) => { setSelectedBudget(budget); setShowDetailsModal(true); };
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 id="budgets-heading" className="text-2xl font-bold text-text-primary-light dark:text-text-primary-dark">Budget Management</h1>
          <p className="text-sm text-text-secondary-light dark:text-text-secondary-dark mt-1">
            {budgets.length} budget{budgets.length !== 1 ? 's' : ''} · {overBudgetCount > 0 ? `${overBudgetCount} over limit` : 'All within limits'}
          </p>
        </div>
        <button
          id="budgets-add-btn"
      ...
      {budgets.length > 0 && (
        <div className="bg-surface-light dark:bg-surface-dark rounded-xl border border-border-light dark:border-border-dark p-5 shadow-sm">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4">
            <div>
              <p className="text-xs text-text-secondary-light dark:text-text-secondary-dark font-medium">Total Limit</p>
              <p className="text-xl font-bold text-text-primary-light dark:text-text-primary-dark mt-0.5">{formatAmount(totalLimit)}</p>
            </div>
            <div>
              <p className="text-xs text-text-secondary-light dark:text-text-secondary-dark font-medium">Total Spent</p>
              <p className={`text-xl font-bold mt-0.5 ${budgetUsagePercent > 90 ? 'text-red-500' : 'text-text-primary-light dark:text-text-primary-dark'}`}>{formatAmount(totalSpent)}</p>
            </div>
            <div>
              <p className="text-xs text-text-secondary-light dark:text-text-secondary-dark font-medium">Remaining</p>
              <p className="text-xl font-bold text-emerald-600 dark:text-emerald-400 mt-0.5">{formatAmount(Math.max(totalLimit - totalSpent, 0))}</p>
            </div>
            <div>
              <p className="text-xs text-text-secondary-light dark:text-text-secondary-dark font-medium">Overall Usage</p>
              <p className={`text-xl font-bold mt-0.5 ${budgetUsagePercent > 90 ? 'text-red-500' : budgetUsagePercent > 70 ? 'text-amber-500' : 'text-text-primary-light dark:text-text-primary-dark'}`}>
          ...
          {overBudgetCount > 0 && (
            <div className="flex items-center gap-2 mt-3 bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 rounded-lg px-3 py-2">
              ...
              <p className="text-xs text-red-700 dark:text-red-300 font-medium">
                {overBudgetCount} budget{overBudgetCount !== 1 ? 's' : ''} exceeded — review your spending.
              </p>
            </div>
          )}
        </div>
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="bg-surface-light dark:bg-surface-dark rounded-xl border border-border-light dark:border-border-dark p-5 animate-pulse space-y-3">
        ...
        <div className="flex flex-col items-center justify-center py-20 gap-4">
          <div className="w-16 h-16 rounded-2xl bg-indigo-50 dark:bg-indigo-900/30 flex items-center justify-center">
            <span className="material-icons text-3xl text-indigo-400">savings</span>
          </div>
          <div className="text-center">
            <p className="font-semibold text-text-primary-light dark:text-text-primary-dark">
              {periodFilter === 'all' ? 'No budgets yet' : `No ${periodFilter} budgets`}
            </p>
            <p className="text-sm text-text-secondary-light dark:text-text-secondary-dark mt-1">
              {periodFilter === 'all' ? 'Create your first budget to start tracking.' : 'Try a different period filter.'}
            </p>
          </div>
          {periodFilter === 'all' && (
            <button
            ...
            return (
              <div
                ...
                className={`bg-surface-light dark:bg-surface-dark rounded-xl shadow-sm border p-5 flex flex-col justify-between transition-all hover:shadow-md ${
                  isExceeded ? 'border-red-300 dark:border-red-700/60' :
                  isNearLimit ? 'border-amber-300 dark:border-amber-700/60' :
                ...
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="font-bold text-text-primary-light dark:text-text-primary-dark leading-tight">{budget.category}</h3>
                        <span className="text-xs text-text-secondary-light dark:text-text-secondary-dark capitalize">{budget.period}</span>
                      </div>
                    </div>
                    <div className="flex items-center gap-1">
                      {isExceeded && <span className="w-2 h-2 rounded-full bg-red-500 animate-pulse" />}
                      {isNearLimit && !isExceeded && <span className="w-2 h-2 rounded-full bg-amber-400" />}
                    </div>
                  </div>
                  ...
                  <div className="space-y-2 mb-4">
                    <div className="flex justify-between text-sm">
                      <span className="text-text-secondary-light dark:text-text-secondary-dark">Spent</span>
                      <span className={`font-semibold ${isExceeded ? 'text-red-500' : isNearLimit ? 'text-amber-500' : 'text-text-primary-light dark:text-text-primary-dark'}`}>
                        {formatAmount(spent)}
                      </span>
                    </div>
                    ...
                    <div className="flex justify-between text-xs text-text-secondary-light dark:text-text-secondary-dark">
                      ...
                      <span className={`font-semibold ${isExceeded ? 'text-red-500' : isNearLimit ? 'text-amber-500' : ''}`}>
                        {percentage}% used
                      </span>
                    </div>
                  </div>
                  ...
                  {isNearLimit && (
                    <div className={`flex items-start gap-2 rounded-lg px-3 py-2 mb-3 ${
                      ...
                      <span className={`material-icons text-sm mt-0.5 ${isExceeded ? 'text-red-500' : 'text-amber-500'}`}>warning</span>
                      <p className={`text-xs font-medium ${isExceeded ? 'text-red-700 dark:text-red-300' : 'text-amber-700 dark:text-amber-300'}`}>
                        {isExceeded
      ...
      {/* ── Modals ─────────────────────────────────────────────────── */}
      <AddBudgetModal isOpen={showAddModal} onClose={() => setShowAddModal(false)} onCreate={async (input) => { await createBudget(input); }} />
      <BudgetDetailsModal isOpen={showDetailsModal} onClose={() => { setShowDetailsModal(false); setSelectedBudget(null); }} budget={selectedBudget} transactions={transactions} />
      <EditBudgetModal isOpen={showEditModal} onClose={() => { setShowEditModal(false); setEditingBudget(null); }} budget={editingBudget} onSave={handleSaveEdit} />
      <DeleteBudgetModal isOpen={!!deletingBudget} category={deletingBudget?.category ?? ''} onConfirm={handleConfirmDelete} onCancel={() => setDeletingBudget(null)} loading={deleteLoading} />
    </div>
  );
};
